...
class ServiceObject {
    ...
    public void updateCurrentTemp(float targetTemp) {
        // 简化的温度变化模拟
        float tempDiff = Math.abs(this.currentTemp - targetTemp);
        float changeRate = 0.5f * fanSpeed.getPriority(); // 风速越高，变化越快

        if (this.currentTemp < targetTemp) {
            this.currentTemp = Math.min(targetTemp, this.currentTemp + changeRate);
        } else {
            this.currentTemp = Math.max(targetTemp, this.currentTemp - changeRate);
        }
    }
    ...
    public float calculateFee() {
        ...
        this.fee = minutes * fanSpeed.getFeeRate();
        return this.fee;
    }
...
class ServiceQueueItem {
    ...
    public void updateServeTime() {
        this.serveTime = serviceObject.getServiceTime();
    }
...
class WaitQueueItem {
    ...
    public WaitQueueItem(RequestObject requestObject) {
        ...
        this.waitTime = 120; // 默认等待2分钟
        this.remainingWaitTime = 120;
    }
    ...
    public int getActualWaitTime() {
        ...
        return (int) ((new Date().getTime() - requestTime.getTime()) / 60000);
    }
...
class DetailRecord {
    ...
    public String toString() {
        return String.format("房间号: %s, 请求时间: %s, 服务开始: %s, 服务结束: %s, " +
                        "服务时长: %d分钟, 风速: %s, 当前费用: %.2f元, 费率: %.2f元/分钟, " +
...
class ScheduleObject {
    ...
    private boolean checkTimeSliceSchedule(RequestObject request) {
        ...
        waitItem.setRemainingWaitTime(timeSlice / 60); // 转换为分钟
    ...
    public void printStatus() {
        ...
        for (WaitQueueItem item : waitQueue) {
            System.out.println("  房间: " + item.getRoomId() +
                    ", 风速: " + item.getFanSpeed().getDescription() +
                    ", 剩余等待时间: " + item.getRemainingWaitTime() + "分钟");
        }
        System.out.println("================\n");
    }
